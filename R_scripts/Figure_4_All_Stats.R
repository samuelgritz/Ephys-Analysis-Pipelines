...
file_name <- '../paper_data/E_I_data/E_I_EPSP_amplitudes_R_format.csv'
...
file_name_ei <- '../paper_data/E_I_data/E_I_amplitudes.csv'
...
for (pathway_code in c("1", "2", "3")) {
  ...
  gabazine_data <- E_I_experiment_long %>% filter(Pathway == pathway_code & Drug == 1)
...
all_anova_df <- bind_rows(all_anova_results) %>%
  select(Analysis, Pathway, Comparison, Effect, `Sum Sq`, `Mean Sq`, NumDF, DenDF, `F value`, P_Value, Significant) %>%
  arrange(Analysis, Pathway, Comparison, Effect)

output_anova <- '../paper_data/E_I_data/Figure_4_All_Stats_ANOVA.csv'
...
output_uncorrected <- '../paper_data/E_I_data/Figure_4_All_Stats_Uncorrected.csv'
...
if (nrow(all_pvalues_df) > 0) {
  ...
  output_corrected <- '../paper_data/E_I_data/Figure_4_All_Stats_FDR_Corrected.csv'
...
output_summary <- '../paper_data/E_I_data/Figure_4_Stats_Summary.csv'
...
if (nrow(all_pvalues_df) > 0) {
  # Get unique analysis/pathway/comparison combinations
  unique_combos <- all_pvalues_df %>%
    select(Analysis, Pathway, Comparison, Main_Effect_p, Interaction_p) %>%
    distinct()
  ...
  for (i in 1:nrow(unique_combos)) {
    ...
    # Check main effect
    main_sig <- ifelse(!is.na(combo$Main_Effect_p) && combo$Main_Effect_p < 0.05, "#", "")
    
    # Check each ISI for post-hoc significance after FDR
    ...
    markers_list[[length(markers_list) + 1]] <- list(
      ...
      Main_Effect_p = combo$Main_Effect_p,
      Interaction_p = combo$Interaction_p
    )
  }
}

markers_df <- bind_rows(markers_list)
output_markers <- '../paper_data/E_I_data/Figure_4_Significance_Markers.csv'
